...
static void papyrus_hw_send_powerup(struct papyrus_sess *sess)
{
	...
	//chencx
//	msleep(30) ;
//	sess->enable_reg_shadow |=0x10 ;
//	stat |= papyrus_hw_setreg(sess, PAPYRUS_ADDR_ENABLE, sess->enable_reg_shadow);

	return;
}
//chencx add
// static void no_need_cfg_send_powerup(struct papyrus_sess *sess, int up)
// {
// 	//uint8_t temp = 0 ;
//	int stat = 0;
//	printk("-------------no_need_cfg_send_powerup 0x%x up=%d-------------\n", sess->enable_reg_shadow, up) ;
//	
//	if(!up) //off
//	{
//	}
...
gpiod_direction_output(sess->vcom_ctl_pin, 0);
	...
	printk("-------------papyrus_hw_send_powerup 0x%x up=%d-------------\n", sess->enable_reg_shadow, up) ;
	...
	printk("-------------no_need_cfg_send_powerup 0x%x up=%d-------------\n", sess->enable_reg_shadow, up) ;
		...
		//papyrus_hw_send_powerup(sess);
			if (!IS_ERR_OR_NULL(sess->pwr_up_pin))
				gpiod_direction_output(sess->pwr_up_pin, 1);
//no_need_cfg_send_powerup(sess, up) ;
			enable_irq(sess->irq);
		} else {
			disable_irq(sess->irq);
		//	papyrus_hw_send_powerdown(sess);
...
static void papyrus_pm_sleep(struct ebc_pmic *pmic)
{
	...
	//chencx
	s->enable_reg_shadow &= 0xef ;
	papyrus_hw_setreg(s, PAPYRUS_ADDR_ENABLE, s->enable_reg_shadow);
	//
